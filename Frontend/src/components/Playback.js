...
export async function playItem(item, syncFunction = () => {}) {
  ...
  var route = (() => {
    switch (item.type) {
      case "playlist":
        return "/api/player/play_playlist";
      case "track":
        return "/api/player/play_song";
      case "album":
        return "/api/player/play_album";
      case "artist":
        return "/api/player/play_artist";
      case "episode":
        return "/api/player/play_song";
...
export async function queueItem(item, syncFunction = () => {}) {
  const axiosInstance = axios.create({ withCredentials: true });
  const response = await axiosInstance.post("/api/player/add_to_queue", {
    song_uri: item.uri,
  });
  syncFunction();
  return response.data;
...
export async function searchSpotify(query, type) {
  const axiosInstance = axios.create({ withCredentials: true });
  const response = await axiosInstance.post("/api/player/search_bar", {
    query: query,
    criteria: type,
  });
  return response.data;
  ...
  try {
    switch (field) {
      ...
      case "release_date":
        switch (track.type) {
          case "track":
            return formatDate(track.album.release_date);
          case "episode":
            return formatDate(track.release_date);
          default:
            return "";
        }
      case "description":
...
function Playback({ syncTrigger = null }) {
  ...
  async function saveVolume(volumeParameter) {
    ...
    const axiosInstance = axios.create({ withCredentials: true });
    const response = await axiosInstance.post("/api/player/volume", {
  ...
  async function changeDevice(newDeviceID) {
    ...
    const axiosInstance = axios.create({ withCredentials: true });
    const response = await axiosInstance.post("/api/player/change_device", {
      uri: newDeviceID,
    });
    const data = response.data;
    syncPlayer();
  ...
  async function getSongParameters(song) {
    ...
    const axiosInstance = axios.create({ withCredentials: true });
    const response = await axiosInstance.post("/api/info/get_song_dict", {
      song_uri: song.uri,
    });
  ...
  function renderPlayerButtons() {
    return (
      <div>
        <img
          id="prevButton"
          style={{ ...songPlayerButtonStyle, left: "20px" }}
        ...
        <img
          id="playButton"
          style={{ ...songPlayerButtonStyle, left: "80px" }}
        ...
        <img
          id="nextButton"
          style={{ ...songPlayerButtonStyle, left: "140px" }}
        ...
        <img
          id="repeatButton"
          style={{ ...songPlayerButtonStyle, left: "200px" }}
        ...
        <img
          id="shuffleButton"
          style={{ ...songPlayerButtonStyle, left: "260px" }}
      ...
      case "track":
        return (
          <div>
            ...
            <p
              style={{ ...headerTextStyle, cursor: "pointer" }}
              onClick={() => playItem(currentTrack.album, syncPlayer)}
            >
              {"Album: " + currentTrack.album.name}
            </p>
            <p style={textStyle}>
              {"Released " + trackData(currentTrack, "release_date")}
            </p>
            <p style={textStyle}>
              {currentTrack.album.total_tracks + " Tracks"}
            </p>
            <p
              style={{ ...headerTextStyle, cursor: "pointer" }}
              onClick={() => playItem(currentTrack.artists[0], syncPlayer)}
            >
      ...
      <div style={expandedPlayerStyle}>
        ...
        <div style={{ width: "32%" }}>
          ...
          <div style={{ ...sectionContainerStyle, textAlign: "center" }}>
            ...
            <p style={{ ...textStyle, fontSize: textSizes.header3 }}>
          ...
          {(() => {
            switch (mode) {
              case "queue":
                return (
                  <div
                    style={{ ...sectionContainerStyle, overflowY: "scroll" }}
                  >
                  ...
                  >
                    ...
                    <select
                      ...
                      onChange={(e) => {
                        ...
                        getSearchResults(searchString, e.target.value);
                      }}
                      style={{
                        ...buttonStyle,
                        width: "max(calc(100% - 270px), 60px)",
                        position: "absolute",
                        top: "10px",
                        right: "130px",
                      }}
                    >
                      <option key={0} value="track">
                        Songs
                      </option>
                      <option key={1} value="artist">
                        Artists
                      </option>
                      <option key={2} value="album">
                        Albums
                      </option>
                      <option key={3} value="playlist">
                        Playlists
                      </option>
                      <option key={4} value="episode">
                        Episodes
                      </option>
                      <option key={5} value="show">
                        Shows
                      </option>
                    </select>
                    ...
                    <ItemList
                      ...
                      data={searchResults}
                      onClick={(index) =>
                        playItem(searchResults[index], syncPlayer)
                      }
                      buttons={
                        searchQueryType === "track" ||
                        searchQueryType === "episode"
                          ? [
                              {
                                width: "40px",
                                value: "+",
                                size: "30px",
                                onClick: (item) => queueItem(item, syncPlayer),
                              },
                            ]
                          : []
                      }
        ...
        <img
          id="expanderButton"
          style={{
            ...songPlayerButtonStyle,
            right: "20px",
            transform: "rotate(180deg)",
          }}
          src={images.expandButton}
    ...
    return (
      <div>
        <div style={songPlayerStyle}>
          ...
          >
            {trackData(currentTrack, "creator")}
          </p>
          <p
            style={{
              color: pulseColors.black,
        ...
        <img
          id="expanderButton"
          style={{ ...songPlayerButtonStyle, right: "20px" }}
          src={images.expandButton}
          onClick={() => setExpanded(true)}
          alt="Expand"
        />
