...
`;
  ...
  // playback 
    ...
    if (randomFriendId && friendsRecentSongs ) {
      try {
        ...
        const response =  axiosInstance.post('/api/games/playback_friends', payload);
  ...
  async function getRandomFriend(friendSongsData) {
    ...
    if (friendSongsData) {
      try {
        ...
        const response = await axiosInstance.post('/api/games/random_friend', payload);
  ...
  const sendPlayerDataToBackend = () => {
    ...
    const backendEndpoint = "/api/games/store_scores";
